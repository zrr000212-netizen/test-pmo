...
周报生成器是一个基于自然语言的智能周报写作助手，能够将零散工作信息整理成结构化周报。
支持多种职业角色（工程师/运营/销售/HR/会计等）和输出格式，纯自然语言驱动，无需任何依赖。
...
- 无需安装任何软件或依赖，纯自然语言驱动
- 需要一个支持对话的 AI 助手（如 Claude、GPT 等）
- 建议搭配 `smart-weekly-report` 和 `email-send` 技能实现完整周报管理流程
...
| 参数 | 说明 | 是否必需 | 示例 |
|------|------|----------|------|
| 职业角色 | 决定周报模板选择 | 是 | 前端工程师、运营、销售 |
| 汇报对象 | 决定报告风格 | 否 | 上级/团队/跨部门 |
| 本周完成 | 已完成的工作内容 | 是 | 完成登录页面重构 |
| 进行中 | 正在进行的工作及进度 | 否 | 首页优化 50% |
| 遇到的问题 | 阻碍和风险 | 否 | 接口联调数据格式不一致 |
| 下周计划 | 下周工作安排 | 是 | 完成支付功能开发 |
| 关键数据 | 量化指标 | 否 | 代码提交12次，PR合并3个 |
...
- [weekly-report-collector 技能 (weekly-report-collector) — 周报收集器
- [email-send 技能 (email-send) — 邮件发送技能
- [gen-week-reporter-email-automation 技能 (gen-week-reporter-email-automation) — 周报自动化流程